...
def impad(img: np.ndarray,
    ...
    img = F.pad(img, pad=(padding[0], padding[2], padding[1], padding[3]), mode=padding_mode, value=pad_val)
...
def imnormalize_(img, mean, std, to_rgb=True):
    ...
    mean = torch.from_numpy(np.float64(mean.reshape(1, 1, -1))).cuda()
    stdinv = torch.from_numpy(1 / np.float64(std.reshape(1, 1, -1))).cuda()
    # if to_rgb:
...
def imresize(
    ...
    if backend == 'pillow':
        ...
        if isinstance(img, np.ndarray):
            ...
            resized_img = F.interpolate(input=img.unsqueeze(0), size=(size[1], size[0]), mode=interpolation)[0]
    if not return_scale:
        return resized_img
    else:
        w_scale = size[0] / w
