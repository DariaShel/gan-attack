...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        path1, path2, path3 = self.local_images[idx].split(' ')
        ...
        params =  get_params(pil_image3.size, self.resize_size, self.crop_size)
        ...
        label_black = transform_label(label_black)
        label_black = np.asarray(label_black)
        
        label_pil = transform_label(pil_image3)
        ...
        if self.down_sample_img:
            ...
            data_dict = {"ref":label_tensor, "low_res":down_sampled_image, "ref_ori":label_tensor_ori, "path": path1, "mask": label_black}
 
            return image_tensor, data_dict

        if random.random() < self.uncond_p:
            label_tensor = th.ones_like(label_tensor)
        data_dict = {"ref":label_tensor, "ref_ori":label_tensor_ori, "path": path1, "mask": label_black}
