...
def get_detector_loss(probs):
    ...
    losses = []
    for prob in probs:
        target = th.full(prob.shape, 0.1)
        losses.append(criterion(prob, target.to(dist_util.dev())))
    return sum(losses) / len(probs)

def get_probs(model_output):
    probs = []
...
class GaussianDiffusion:
    ...
    def training_losses(self, model, detector, x_start, t, vgg, adv, mask_overlay, model_kwargs=None, noise=None):
        """
        ...
        if adv is not None:
            gen = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output).clamp(-1, 1)
            overlay_img = th.where(mask_overlay.unsqueeze(0).to(dist_util.dev()) == 1, gen, x_start)

            terms['gan_loss_attack'] = adv.gan_loss(overlay_img)
            pred_det = inference_detector(detector, denormalize(overlay_img[0]).to(dist_util.dev()))
            # result_numpy = det_output2numpy(pred_det)
            probs = get_probs(pred_det)
            if len(probs) > 0:
                terms['det_attack'] = get_detector_loss(probs)
            else:
                terms['det_attack'] = 0

            terms['loss'] += terms['gan_loss_attack'] + terms['det_attack']
